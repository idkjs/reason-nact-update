...
type persistenceEngine = NactBindings.persistenceEngine;

type untypedRef = NactBindings.actorRef;

type actorRef('msg) =
  | ActorRef(NactBindings.actorRef);

module Interop = {
  let fromUntypedRef = reference => ActorRef(reference);
  let toUntypedRef = (ActorRef(reference)) => reference;
  let dispatch = NactBindings.dispatch;
  let dispatchWithSender = NactBindings.dispatchWithSender;
};

type actorPath =
  | ActorPath(NactBindings.actorPath);
...
let mapCtx = (untypedCtx: NactBindings.ctx) => {
...
let mapPersistentCtx = (untypedCtx: NactBindings.persistentCtx('incoming)) => {
  name: untypedCtx##name,
  self: ActorRef(untypedCtx##self),
  parent: ActorRef(untypedCtx##parent),
  path: ActorPath(untypedCtx##path),
...
let mapSupervisionCtx = (untypedCtx: NactBindings.supervisionCtx) => {
  name: untypedCtx##name,
  self: ActorRef(untypedCtx##self),
  parent: ActorRef(untypedCtx##parent),
  path: ActorPath(untypedCtx##path),
...
let spawn =
  ...
  let untypedRef = NactBindings.spawn(parent, f, fromOption(name), options);
  ActorRef(untypedRef);
};
...
let spawnStateless = (~name=?, ~shutdownAfter=?, ActorRef(parent), func) => {
  ...
  let untypedRef =
    NactBindings.spawnStateless(parent, f, fromOption(name), options);
  ActorRef(untypedRef);
};
...
let spawnPersistent =
  ...
  let options: NactBindings.persistentActorOptions('msg, 'parentMsg, 'state) = {
  ...
  let untypedRef =
    NactBindings.spawnPersistent(parent, f, key, fromOption(name), options);
  ActorRef(untypedRef);
};
...
let persistentQuery =
  ...
  let stateEncoder =
    stateEncoder->(Belt.Option.getWithDefault(unsafeEncoder));
  let encoder = encoder->(Belt.Option.getWithDefault(unsafeEncoder));
  let options: NactBindings.persistentQueryOptions('msg, 'state) = {
  ...
  NactBindings.persistentQuery(actor, f, key, options);
};

let stop = (ActorRef(reference)) => NactBindings.stop(reference);

let dispatch = (ActorRef(recipient), msg) =>
  NactBindings.dispatch(recipient, msg);

let nobody = () => ActorRef(NactBindings.nobody());
...
let start = (~name: option(string)=?, ~persistenceEngine=?, ()) => {
  let plugins =
    ...
    | Some(engine) => [NactBindings.configurePersistence(engine)]
    | None => []
    };
  let plugins =
    switch (name) {
  ...
  switch (plugins) {
  | [a, b, ..._] => ActorRef(NactBindings.start([|a, b|]))
  | [a] => ActorRef(NactBindings.start([|a|]))
  | [] => ActorRef(NactBindings.start([||]))
  };
};
...
let query = (~timeout: int, ActorRef(recipient), msgF) => {
  ...
  NactBindings.query(recipient, f, timeout)
